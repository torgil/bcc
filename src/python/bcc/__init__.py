        ...
        if lib.bcc_symcache_resolve(self.cache, addr, psym) < 0:
            if sym.module and sym.offset:
                return (None, sym.offset,
                        to_string(ct.cast(sym.module, ct.c_char_p).value))
            return (None, addr, None)
        return (to_string(sym.demangle_name), sym.offset,
                to_string(ct.cast(sym.module, ct.c_char_p).value))

    def resolve_name(self, module, name):
        addr = ct.c_ulonglong()
        if lib.bcc_symcache_resolve_name(
                    self.cache, to_bytes(module) if module else None,
                    to_bytes(name), ct.pointer(addr)) < 0:
            return -1
        return addr.value
...
class BPF(object):
    ...
    def __init__(self, src_file="", hdr_file="", text=None, cb=None, debug=0,
        ...
        for i, s in enumerate(cflags): cflags_array[i] = to_bytes(s)
        if text:
        ...
        if text:
            self.module = lib.bpf_module_create_c_from_string(to_bytes(text),
                    self.debug, cflags_array, len(cflags_array))
        else:
            src_file = BPF._find_file(src_file)
            hdr_file = BPF._find_file(hdr_file)
            if src_file.endswith(".b"):
                self.module = lib.bpf_module_create_b(to_bytes(src_file),
                        to_bytes(hdr_file), self.debug)
            else:
                self.module = lib.bpf_module_create_c(to_bytes(src_file),
                        self.debug, cflags_array, len(cflags_array))
    ...
    def load_funcs(self, prog_type=KPROBE):
        ...
        for i in range(0, lib.bpf_num_functions(self.module)):
            func_name = to_string(lib.bpf_function_name(self.module, i))
    ...
    def load_func(self, func_name, prog_type):
        ...
        if not lib.bpf_function_start(self.module, to_bytes(func_name)):
        ...
        while True:
            ...
            fd = lib.bpf_prog_load(prog_type,
                    lib.bpf_function_start(self.module, to_bytes(func_name)),
                    lib.bpf_function_size(self.module, to_bytes(func_name)),
        ...
        if self.debug & DEBUG_BPF and log_buf.value:
            print(to_string(log_buf.value), file=sys.stderr)
    ...
    def dump_func(self, func_name):
        ...
        start, = lib.bpf_function_start(self.module, to_bytes(func_name)),
        size, = lib.bpf_function_size(self.module, to_bytes(func_name)),
    ...
    def get_table(self, name, keytype=None, leaftype=None, reducer=None):
        map_id = lib.bpf_table_id(self.module, to_bytes(name))
        map_fd = lib.bpf_table_fd(self.module, to_bytes(name))
        if map_fd < 0:
            raise KeyError
        if not keytype:
            key_desc = lib.bpf_table_key_desc(self.module, to_bytes(name))
            if not key_desc:
                raise Exception("Failed to load BPF Table %s key desc" % name)
            keytype = BPF._decode_table_type(json.loads(to_string(key_desc)))
        if not leaftype:
            leaf_desc = lib.bpf_table_leaf_desc(self.module, to_bytes(name))
            if not leaf_desc:
                raise Exception("Failed to load BPF Table %s leaf desc" % name)
            leaftype = BPF._decode_table_type(json.loads(to_string(leaf_desc)))
    ...
    def attach_raw_socket(fn, dev):
        ...
        sock = lib.bpf_open_raw_sock(to_bytes(dev))
        if sock < 0:
            errstr = os.strerror(ct.get_errno())
    ...
    def attach_kprobe(self, event="", fn_name="", event_re="",
        ...
        res = lib.bpf_attach_kprobe(fn.fd, 0, to_bytes(ev_name),
                to_bytes(event), pid, cpu, group_fd,
    ...
    def detach_kprobe(self, event):
        ...
        res = lib.bpf_detach_kprobe(to_bytes(ev_name))
        if res < 0:
    ...
    def attach_kretprobe(self, event="", fn_name="", event_re="",
        ...
        res = lib.bpf_attach_kprobe(fn.fd, 1, to_bytes(ev_name),
                to_bytes(event), pid, cpu, group_fd,
                self._reader_cb_impl, ct.cast(id(self), ct.py_object))
        res = ct.cast(res, ct.c_void_p)
        if not res:
    ...
    def attach_xdp(dev, fn):
        ...
        if not isinstance(fn, BPF.Function):
            raise Exception("arg 1 must be of type BPF.Function")
        res = lib.bpf_attach_xdp(to_bytes(dev), fn.fd)
        if res < 0:
            err_no = ct.get_errno()
    ...
    def remove_xdp(dev):
        ...
        '''
        res = lib.bpf_attach_xdp(to_bytes(dev), -1)
        if res < 0:
            errstr = os.strerror(ct.get_errno())
    ...
    def _check_path_symbol(cls, module, symname, addr, pid):
        ...
        if lib.bcc_resolve_symname(to_bytes(module),
                to_bytes(symname), addr or 0x0, c_pid, psym) < 0:
            if not sym.module:
                raise Exception("could not find library %s" % module)
            lib.bcc_procutils_free(sym.module)
            raise Exception("could not determine address of symbol %s" % symname)
        module_path = to_string(ct.cast(sym.module, ct.c_char_p).value)
        lib.bcc_procutils_free(sym.module)
        return module_path, sym.offset

    @staticmethod
    def find_library(libname):
        res = lib.bcc_procutils_which_so(to_bytes(libname), 0)
        if not res:
            return None
        libpath = to_string(ct.cast(res, ct.c_char_p).value)
    ...
    def attach_tracepoint(self, tp="", tp_re="", fn_name="", pid=-1,
        ...
        res = lib.bpf_attach_tracepoint(fn.fd, to_bytes(tp_category),
                to_bytes(tp_name), pid, cpu, group_fd,
                self._reader_cb_impl, ct.cast(id(self), ct.py_object))
        res = ct.cast(res, ct.c_void_p)
        if not res:
    ...
    def detach_tracepoint(self, tp=""):
        ...
        (tp_category, tp_name) = tp.split(':')
        res = lib.bpf_detach_tracepoint(to_bytes(tp_category),
                                        to_bytes(tp_name))
        if res < 0:
    ...
    def get_user_functions_and_addresses(name, sym_re):
        ...
        def sym_cb(sym_name, addr):
            dname = to_string(sym_name)
            if re.match(sym_re, dname):
                addresses.append((dname, addr))
            return 0

        res = lib.bcc_foreach_symbol(to_bytes(name), _SYM_CB_TYPE(sym_cb))
        if res < 0:
    ...
    def attach_uprobe(self, name="", sym="", sym_re="", addr=None,
        ...
        res = lib.bpf_attach_uprobe(fn.fd, 0, to_bytes(ev_name),
                to_bytes(path), addr, pid, cpu, group_fd,
                self._reader_cb_impl, ct.cast(id(self), ct.py_object))
        res = ct.cast(res, ct.c_void_p)
        if not res:
    ...
    def detach_uprobe(self, name="", sym="", addr=None, pid=-1):
        ...
        res = lib.bpf_detach_uprobe(to_bytes(ev_name))
        if res < 0:
    ...
    def attach_uretprobe(self, name="", sym="", sym_re="", addr=None,
        ...
        res = lib.bpf_attach_uprobe(fn.fd, 1, to_bytes(ev_name),
                to_bytes(path), addr, pid, cpu, group_fd,
                self._reader_cb_impl, ct.cast(id(self), ct.py_object))
        res = ct.cast(res, ct.c_void_p)
        if not res:
    ...
    def _trace_autoload(self):
        for i in range(0, lib.bpf_num_functions(self.module)):
            func_name = to_string(str(lib.bpf_function_name(self.module, i)))
    ...
    def cleanup(self):
        for k, v in list(self.open_kprobes.items()):
            ...
            if isinstance(k, str):
                to_bytes(lib.bpf_detach_kprobe(str(k)))
            self._del_kprobe(k)
        for k, v in list(self.open_uprobes.items()):
            lib.perf_reader_free(v)
            to_bytes(lib.bpf_detach_uprobe(str(k)))
            self._del_uprobe(k)
        for k, v in self.open_tracepoints.items():
            lib.perf_reader_free(v)
            (tp_category, tp_name) = k.split(':')
            lib.bpf_detach_tracepoint(to_bytes(tp_category),
                    to_bytes(tp_name))
