...
TableStorage::~TableStorage() {}
void TableStorage::Init(unique_ptr<TableStorageImpl> impl) {
  impl_ = move(impl);
}
...
bool TableStorage::Delete(const Path &path) {
  return impl_->Delete(path.to_string());
}
...
TableStorage::iterator TableStorage::end() {
  return impl_->end();
}
...
TableStorage::iterator::~iterator() {}
TableStorage::iterator::iterator(iterator &&that) {
  *this = move(that);
}
...
TableStorage::iterator::reference TableStorage::iterator::operator*() const {
  return **impl_;
}
TableStorage::iterator::pointer TableStorage::iterator::operator->() const {
  return &**impl_;
}
